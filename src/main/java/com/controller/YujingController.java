    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(yujing !=null){
            ...
            BeanUtils.copyProperties( yujing , view );//把实体数据重构到view中
                ...
                //级联表
                XueshengEntity xuesheng = xueshengService.selectById(yujing.getXueshengId());
                if(xuesheng != null){
    ...
    */
    ...
    public R save(@RequestBody YujingEntity yujing, HttpServletRequest request){
        ...
        if(false)
            ...
            yujing.setXueshengId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<YujingEntity> queryWrapper = new EntityWrapper<YujingEntity>()
            .eq("xuesheng_id", yujing.getXueshengId())
            .eq("kecheng_types", yujing.getKechengTypes())
            ;
...
//                            yujingEntity.setXueshengId(Integer.valueOf(data.get(0)));   //学生 要改的
//                            yujingEntity.setKechengTypes(Integer.valueOf(data.get(0)));   //课程 要改的
